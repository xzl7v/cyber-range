...
The session defaults are 2 GiB shared memory, 1280x720, 30 FPS, bandwidth-first KasmVNC quality, disabled XFCE compositing, fixed `PublicIP=127.0.0.1` to avoid STUN delays in isolated networks, and server-controlled desktop sizing. Lab manifests use the `cyber-range/kali-optimized:v4` Core Kali layer, which supplies XFCE, KasmVNC, the browser, file manager, and only the current lab tools. Software rendering is the default; the runtime requests NVIDIA acceleration only when Docker advertises the NVIDIA runtime. The legacy `student-node` service is not used by dynamic lab sessions.
